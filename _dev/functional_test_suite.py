...
def run_comprehensive_tests():
    ...
    try:
        clear_scene()
        bpy.context.scene.lsd_pg_ai_props.ai_source = 'FREE'
        bpy.context.scene.lsd_pg_ai_props.api_prompt = "Generate a six-wheeled rover with a robotic arm"
        res = bpy.ops.lsd.execute_ai_prompt()
        if 'FINISHED' in res:
    ...
    for t in templates:
        ...
        try:
            clear_scene()
            bpy.context.scene.lsd_pg_ai_props.robot_template = t
    ...
    try:
        ...
        res = bpy.ops.lsd.create_part()
        if 'FINISHED' in res:
            obj = bpy.context.active_object
            print(f"[SUCCESS] Created part: {obj.name}")
            # Test Baking
            print("  Testing Bake...")
            res_bake = bpy.ops.lsd.bake_mesh()
    ...
    try:
        ...
        # Calculate COM
        res_com = bpy.ops.lsd.calculate_center_of_mass()
        print(f"  COM Calculation: {res_com}")
        print(f"  Resulting COM: {pbone.lsd_pg_kinematic_props.inertial.center_of_mass}")
        
        # Calculate Inertia
        pbone.lsd_pg_kinematic_props.collision.shape = 'BOX'
        pbone.lsd_pg_kinematic_props.inertial.mass = 1.0
        res_inertia = bpy.ops.lsd.calculate_inertia()
        print(f"  Inertia Calculation: {res_inertia}")
        print(f"  Resulting ixx: {pbone.lsd_pg_kinematic_props.inertial.ixx}")
