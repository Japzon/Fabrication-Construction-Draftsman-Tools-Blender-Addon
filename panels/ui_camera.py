...
class FCD_PT_Camera_Cinematography:
    """
    ...
    bl_idname = "VIEW3D_PT_fcd_camera"

    @classmethod
    def poll(cls, context: bpy.types.Context) -> bool:
        # This panel is only drawn if its corresponding visibility toggle is enabled.
        return getattr(context.scene, "fcd_panel_enabled_camera", True)
    ...
    def draw(layout: bpy.types.UILayout, context: bpy.types.Context) -> None:
        ...
        box, is_expanded = ui_common.draw_panel_header(
            ...
            "fcd_show_panel_camera", 
            "fcd_panel_enabled_camera"
        )
        ...
        if is_expanded:
            ...
            row.scale_y = 1.4
            row.operator("fcd.camera_setup", text="Apply Setup", icon='CHECKMARK')
            row.operator("fcd.camera_look_through", text="View", icon='HIDE_OFF')
