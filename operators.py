...
class FCD_OT_CalculateCenterOfMass(bpy.types.Operator):
    ...
    def poll(cls, context: bpy.types.Context) -> bool:
        ...
        return obj is not None and (obj.name.startswith("COLL_") or hasattr(obj, "fcd_pg_mech_props"))
...
class FCD_OT_ApplyJointSettings(bpy.types.Operator):
    ...
    def execute(self, context: bpy.types.Context) -> Set[str]:
        ...
        if not scene.fcd_viz_gizmos:
            msg += " (Note: Gizmos are HIDDEN in UI)"
        if scene.fcd_placement_mode:
...
def update_material_merge_trigger(self, context):
    ...
    if context.active_object:
        bpy.app.timers.register(lambda: (bpy.ops.fcd.material_merge() and None), first_interval=0.01)
    ...
    for cls in CLASSES:
        if hasattr(cls, 'bl_rna'):
            try:
                ...
                print(f"FCD Operator Register Warning: Could not register {cls.__name__}: {e}")
