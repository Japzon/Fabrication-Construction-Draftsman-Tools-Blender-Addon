...
powershell -Command "Get-Process cmd | Where-Object { $_.MainWindowTitle -like '*LSD_BLENDER_DEV_CONSOLE*' } | Stop-Process -Force -ErrorAction SilentlyContinue"
taskkill /F /IM blender.exe /T 2>nul

:: Set a unique title for the current session to allow cleanup on the next run
title LSD_BLENDER_DEV_CONSOLE
